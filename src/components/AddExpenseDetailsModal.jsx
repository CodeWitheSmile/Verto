...
const AddExpenseDetailsModal = ({
  ...
  useEffect(() => {
    if (editData) {
      setForm({
        ...
        paymentMode: editData.payment_source === "CASH" ? "cash" : "bank",
  ...
  const setCostHead = (head, value) => {
    ...
    setForm((p) => ({
      ...p,
      costHeadBreakup: { ...p.costHeadBreakup, [head]: num },
    }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        employee_name:
          form.paidToType === "Employee" ? form.employeeName : null,
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-6">
              ...
              <section>
                ...
                <div className="bg-gray-50 rounded-xl p-4 divide-y divide-gray-100">
                  ...
                  <FieldRow
                    ...
                    required
                    hint="Internal = Verto expenses, Client = billable/client linked"
                  >
                    <div className="flex gap-3 flex-wrap">
                      {["Internal", "Client"].map((opt) => (
                        <TabBtn
                          key={opt}
                          active={form.expenseNature === opt}
                          color="orange"
                          onClick={() => {
                            setField("expenseNature", opt);
                            setField("isBillable", opt === "Client");
                          }}
                        >
                          {opt}
                        </TabBtn>
                  ...
                  {form.expenseNature === "Client" && (
                    <FieldRow label="Billable?">
                      <div className="flex gap-3 pt-1">
                        <TabBtn
                          active={form.isBillable}
                          color="emerald"
                          onClick={() => setField("isBillable", true)}
                        >
                          Billable
                        </TabBtn>
                        <TabBtn
                          active={!form.isBillable}
                          color="slate"
                          onClick={() => setField("isBillable", false)}
                        >
                          Non Billable
                        </TabBtn>
                      </div>
                  ...
                  {form.isBillable && form.expenseNature === "Client" && (
                    <FieldRow
                      ...
                      hint="Invoice this expense is billed against"
                      highlight
                    >
                      {invoice ? (
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                          ...
                          <p className="text-xs text-gray-500">
                            {invoice.client_name} — auto-linked
                          </p>
                        </div>
                      ) : selectedInvoice ? (
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`w-full border rounded-lg pl-9 pr-4 py-2.5 text-sm text-black bg-white focus:outline-none focus:ring-2 focus:ring-orange-400 ${
                              errors.invoiceId
                                ? "border-red-400"
                                : "border-gray-200"
                            }`}
                  ...
                  {/* Paid To */}
                  <FieldRow
                    label="Paid To / Vendor / Emp"
                    hint="Who received this payment"
                  >
                    <div className="space-y-2">
                      <div className="flex gap-2">
                        <TabBtn
                          active={form.paidToType === "Vendor"}
                          color="orange"
                          onClick={() => setField("paidToType", "Vendor")}
                        >
                          <Building2 className="w-3.5 h-3.5 inline mr-1" />
                          Vendor
                        </TabBtn>
                        <TabBtn
                          active={form.paidToType === "Employee"}
                          color="blue"
                          onClick={() => setField("paidToType", "Employee")}
                        >
                      ...
                      {form.paidToType === "Vendor" && (
                        <div className="space-y-2">
                          <label className="flex items-center gap-1.5 text-xs text-gray-500 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={form.manualVendor}
                              onChange={(e) =>
                                setField("manualVendor", e.target.checked)
                              }
                              className="rounded"
                            />
                              ...
                              <div className="border border-gray-200 rounded-lg max-h-36 overflow-y-auto">
                                {vendors
                                  .filter((v) =>
                                    v.vendor_name
                                      ?.toLowerCase()
                                      .includes(form.vendorName.toLowerCase())
                                  )
                                  .map((v) => (
                                    <button
                                      key={v.id}
                                      type="button"
                                      onClick={() =>
                                        setField("vendorName", v.vendor_name)
                                      }
                            ...
                            <div className="relative">
                              <InputField
                                ...
                                onChange={(v) => {
                                  ...
                                  if (match)
                                    setField("employeeName", match.name);
                                }}
                                placeholder="EMP001..."
                              />
                                ...
                                <div className="absolute z-20 top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                                  {employees
                                    .filter((e) =>
                                      e.emp_code
                                        ?.toLowerCase()
                                        .includes(form.empCode.toLowerCase())
                                    )
                                    .map((e) => (
                                      <button
                                        key={e.id}
                                        type="button"
                                        onClick={() => {
                                          setField("empCode", e.emp_code);
                                          setField("employeeName", e.name);
                                        }}
                  ...
                  {isPettyCashPayHead && (
                    ...
                    >
                      ...
                      <p className="text-xs text-violet-700 font-medium leading-relaxed">
                        <strong>Petty Cash Pay Head:</strong> This expense
                        amount will be automatically added to the Petty Cash
                        ledger via a DB trigger. Manage petty cash history &amp;
                        top-ups from the{" "}
                        <button
                          type="button"
                    ...
                    <div className="space-y-3">
                      <div className="flex gap-2 flex-wrap">
                        {[
                          ...
                          <TabBtn
                            key={opt.value}
                            active={form.paymentMode === opt.value}
                            color={opt.color}
                      ...
                      <FieldRow label="Current Stock Status">
                        <div className="flex gap-2 flex-wrap">
                          {ASSET_STOCK_STATUS.map((s) => (
                            <TabBtn
                              key={s}
                              active={form.assetStockStatus === s}
                              color="blue"
                              onClick={() => setField("assetStockStatus", s)}
                            >
                              {s}
                            </TabBtn>
                          ))}
                  ...
                  >
                    <div className="bg-sky-50 border border-sky-200 rounded-xl p-3">
                      <p className="text-xs text-sky-700 font-medium">
                        💳 <strong>CC Bill:</strong> Ensure Month of Expense,
                        Payment Amount, Description, Date of Pay and Bank are
                        all filled — these represent the credit card statement
                        payment.
                      </p>
                    </div>
                  </motion.section>
              ...
              {form.isBillable && (
                <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3">
                  <p className="text-xs text-indigo-700 font-medium">
                    ✅ Billable Expense: ₹{form.transferAmount || "0"} will be
                    added to invoice outstanding — client owes this amount.
                  </p>
                </div>
              )}
              ...
              >
                {form.paymentMode === "bank" &&
                  "🏦 Bank Transfer → bank_entries (debit) will be auto-created by trigger"}
                {form.paymentMode === "cash" &&
                  "💵 Cash → only software_entries will be created (no bank entry)"}
              </div>
            </div>
            {/* end body */}
...
const SectionTitle = ({ color, label, badge, inline }) => {
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      <div
        className={`w-1 h-5 ${colors[color] || "bg-gray-400"} rounded-full`}
      />
      <h4 className="font-semibold text-gray-800 text-sm uppercase tracking-wider">
        {label}
      </h4>
